...
{
    [AddComponentMenu("UI/Smooth Scroll Rect", 38)]
    [SelectionBase]
    [ExecuteAlways]
    [DisallowMultipleComponent]
    [RequireComponent(typeof(RectTransform))]
    public class SmoothScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        /// <summary>
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            if (m_UseLerp)
            {
                SetTargetAnchoredPosition(position);
            }
            else
            {
                SetContentAnchoredPosition(position);
        ...
        protected virtual void SetContentAnchoredPosition(Vector2 position)
        {
            ...
            if (position != m_Content.anchoredPosition)
            {
                ...
                _targetDrag = position;
                UpdateBounds();
            }
        }

        [SerializeField] private float m_ScrollLerpSensitivity = 10;
        [SerializeField] private bool m_UseLerp = true;
        private Vector2 _targetDrag;
        
        
        protected virtual void SetTargetAnchoredPosition(Vector2 position)
        {
            if (!m_Horizontal)
                position.x = m_Content.anchoredPosition.x;
            if (!m_Vertical)
        ...
        {
            m_Content.anchoredPosition = Vector2.Lerp(m_Content.anchoredPosition, _targetDrag,
                Time.unscaledDeltaTime * m_ScrollLerpSensitivity);
